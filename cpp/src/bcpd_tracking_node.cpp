...
sensor_msgs::ImagePtr Callback(const sensor_msgs::ImageConstPtr& image_msg, const sensor_msgs::PointCloud2ConstPtr& pc_msg) {
    ...
    if (cloud->width != 0 && cloud->height != 0) {
        ...
        pcl::PointCloud<pcl::PointXYZ> bcpd_pc;
        for (int i = 0; i < Y.rows(); i++) {
            pcl::PointXYZ temp;
            temp.x = Y(i, 0);
            temp.y = Y(i, 1);
            temp.z = Y(i, 2);
            bcpd_pc.points.push_back(temp);
        }

        pcl::PCLPointCloud2 result_pc_pclpoincloud2;
        
        pcl::toPCLPointCloud2(bcpd_pc, result_pc_pclpoincloud2);
...
int main(int argc, char **argv) {
    ...
    nh.getParam("/bcpd/use_eval_rope", use_eval_rope);
    ...
    // bcpd point cloud topic
    result_pc_pub = nh.advertise<sensor_msgs::PointCloud2>("/bcpd_results_pc", pub_queue_size);
